...
export default function Home({ popularMovies }) {
  ...
  // قنوات البث الرياضي المباشر الصافية المضافة يدويّاً
  const liveChannels = [
    { id: 'sports-live-1', name: 'الرياضية مباشر - سيرفر أصيل ⚽', poster: 'https://via.placeholder.com/300x450/111/fff?text=LIVE+SPORTS' }
  ];
  ...
  return (
    <div style={{ backgroundColor: '#050505', color: 'white', minHeight: '100vh', padding: '20px', fontFamily: 'sans-serif', direction: 'rtl' }}>
      ...
      {/* 🔴 قسم البث المباشر والرياضة الجديد */}
      <h2 style={{ color: '#fff', borderRight: '4px solid #e50914', paddingRight: '10px', marginBottom: '20px' }}>🔴 البث المباشر والرياضة</h2>
      <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap', marginBottom: '50px' }}>
        {liveChannels.map((channel) => (
          ...
          >
            <img src={channel.poster} alt={channel.name} style={{ width: '100%', height: '240px', objectFit: 'cover' }} />
            <div style={{ padding: '10px', fontSize: '14px', fontWeight: 'bold', textAlign: 'center' }}>{channel.name}</div>
          </div>
        ))}
      </div>

      {/* 🎬 قسم الأفلام والمسلسلات */}
      <h2 style={{ color: '#fff', borderRight: '4px solid #e50914', paddingRight: '10px', marginBottom: '20px' }}>🎬 الأفلام والمسلسلات المقترحة</h2>
